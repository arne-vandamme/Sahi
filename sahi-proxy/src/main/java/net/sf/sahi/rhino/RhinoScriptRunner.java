...
//		long initialFreeMemory = Runtime.getRuntime().freeMemory();
		...
		try {
			...
			if ( browserException != null ) {
				...
				this.scriptStatus = Status.FAILURE;
				report.addResult( "ERROR " + "\n" + stackTrace, ResultType.ERROR, script.getLineDebugInfo(
						ee.lineNumber() - 1 ), ee.details() );
			}
			setHasError();
			...
			if ( recoveryScript != null ) {
				try {
					...
					if ( browserException != null ) {
						...
						this.scriptStatus = Status.FAILURE;
						report.addResult( "ERROR ", ResultType.ERROR, script.getDebugInfo( ee2.lineNumber() - 1 ),
						                  ee2.details() );
					}
				}
				finally {
	...
	public void markStepDone( String stepId, ResultType type, String failureMessage ) {
		...
		if ( stepId.equals( "" + this.counter ) ) {
			if ( type == ResultType.ERROR ) {
				this.scriptStatus = Status.FAILURE;
				if ( !this.stopOnError ) {
					report.addResult( SahiScript.stripSahiFromFunctionNames( step ), type, debugInfo,
					                  failureMessage + "\n" + stackTrace );
				}
			}
			else {
