...
public class Suite
{
	...
	public SimpleHttpResponse executeTestInSingleSession( final HttpRequest request ) throws Exception {
		...
		Status status = suite.executeTestForSingleSession( request.getParameter( "testName" ), request.getParameter(
				"startURL" ) );
	...
	private SahiTestSuite getSuite( final HttpRequest request ) {
		...
		final SahiTestSuite suite = prepareSuite( suitePath, base, browser, session.id(), browserOption,
		                                          browserProcessName, threads, useSystemProxy, isSingleSession,
		                                          request );
		return suite;
	}
	...
	private SahiTestSuite getPreconfiguredBrowserSuite( final HttpRequest request ) {
		...
		if ( browserType != null ) {
			suite = prepareSuite( suitePath, base, browserType.path(), session.id(), browserType.options(),
			                      browserType.processName(), ( "" + threads ), browserType.useSystemProxy(),
			                      isSingleSession, request );
		}
		return suite;
	}
	...
	private SahiTestSuite prepareSuite( String suitePath,
	                                    String base,
	                                    String browser,
	                                    String sessionId,
	                                    String browserOption,
	                                    String browserProcessName,
	                                    String threadCapacity,
	                                    boolean useSystemProxy,
	                                    boolean isSingleSession,
	                                    HttpRequest request ) {
		final SahiTestSuite suite = new SahiTestSuite( net.sf.sahi.config.Configuration.getAbsoluteUserPath(
				suitePath ),
