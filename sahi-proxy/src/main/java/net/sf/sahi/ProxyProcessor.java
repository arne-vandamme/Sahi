...
public class ProxyProcessor implements Runnable
{
		...
		try {
			...
			if ( uri != null ) {
				...
				if ( _s_ != -1 && ( q == -1 || ( q > _s_ ) ) ) {
					...
					if ( isHostTheProxy( requestFromBrowser.host() ) && requestFromBrowser.port() == Configuration
							.getPort() ) {
						processLocally( uri, requestFromBrowser );
					}
					else if ( uri.indexOf( "favicon.ico" ) != -1 ) {
						sendResponseToBrowser( new HttpFileResponse(
								Configuration.getHtdocsRoot() + "spr/favicon.ico" ) );
					}
					else {
	...
	private void processAsProxy( HttpRequest requestFromBrowser ) throws Exception {
		...
		Date time = new Date();
		TrafficLogger.createLoggerForThread( fileName, "unmodified", Configuration.isUnmodifiedTrafficLoggingOn(),
		                                     time );
