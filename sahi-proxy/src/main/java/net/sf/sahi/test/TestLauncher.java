...
public class TestLauncher
{
	...
	public void execute( Session session, boolean async, boolean setDefaultReporters ) throws Exception {
		...
		scriptRunner = new RhinoScriptRunner( new ScriptFactory().getScript( scriptName ), session.getSuite(), this,
		                                      setDefaultReporters );
		...
		if ( async ) {
			...
			scriptRunner.executeAndWait(); // seems not to be used?
		}
	}
	...
	private void launchBrowser() throws Exception {
		browserOption = ( browserOption == null ) ? "" : browserOption.replaceAll( "[$]threadNo", "" + threadNo )
		                                                              .replaceAll( "[$]userDir",
		                                                                           Configuration.getAbsoluteUserPath(
				                                                                           "." ).replace( '\\', '/' ) );
