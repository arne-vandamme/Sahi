	...
	public void run() {
		...
		try {
			...
			try {
				...
				sendResponseToBrowser( new NoCacheHttpResponse( 200, "OK",
				                                                "<script>location.href='" + uri + "/index.htm'</script>" ) );
			}
			catch ( IOException e ) {
			...
			try {
				sendResponseToBrowser( new NoCacheHttpResponse( 404, "FileNotFound",
