...
public class Driver
{
	...
	private void launchBrowser( final HttpRequest request ) throws Exception {
		...
		launch( browser, browserProcessName, browserOption, "true".equals( request.getParameter( "useSystemProxy" ) ),
		        request );
	}
	...
	private void launch( String browser,
...
//		
		...
		String url = "http://" + Configuration.getCommonDomain() + "/_s_/dyn/Driver_start?sahisid="
				...
				+ "&startUrl="
				+ Utils.encode(
				"http://" + Configuration.getCommonDomain() + "/_s_/dyn/Driver_initialized?startUrl=" + Utils.encode(
						startUrl ) );
...
//    		}
	...
	public HttpResponse initialized( final HttpRequest request ) {
		...
		HttpFileResponse httpFileResponse = new HttpFileResponse( Configuration.getHtdocsRoot() + "spr/initialized.htm",
