...
public class SessionState
{
	...
	private String getXHRReadyStatesToWaitFor( Session session ) {
		...
		if ( states == null ) {
			states = Configuration.xhrReadyStatesToWaitFor();
		}
		return states;
	}
	...
	public HttpResponse execute( final HttpRequest request ) {
		...
		props.setProperty( "chromeExplicitCheckboxRadioToggle",
		                   "" + Configuration.getChromeExplicitCheckboxRadioToggle() );
		...
		if ( scriptRunner != null && scriptRunner.getScript() != null ) {
			props.setProperty( "scriptPath", Utils.escapeDoubleQuotesAndBackSlashes(
					Utils.escapeDoubleQuotesAndBackSlashes( scriptRunner.getScript().getFilePath() ) ) );
	...
	private HttpResponse addSahisidCookie( final HttpResponse httpResponse,
		...
		httpResponse.addHeader(
				"P3P",
				"policyref=\"http://" + Configuration
						.getCommonDomain() + "/p3p.xml\", CP=\"NON DSP COR CURa ADMa DEVa CUSa TAIa OUR SAMa IND\"" );
