...
public class HttpModifiedResponse extends StreamingHttpResponse
{
	...
	public HttpModifiedResponse( final StreamingHttpResponse response,
	                             final boolean isSSL,
		...
		if ( responseCode < 300 || responseCode >= 400 ) { // Response code other than 3xx
			...
			if ( html ) {
				if ( Configuration.addCharacterFilter() ) {
					addFilter( new CharacterFilter( charset ) );
				}
				if ( Configuration.addJSModifierFilter() ) {
					addFilter( new JSModifierFilter( charset ) );
				}
				if ( Configuration.addHTMLModifierFilter() ) {
					addFilter( new HTMLModifierFilter( charset, isXHTML(),
					                                   isSSL ) );
				}
...
//            	addFilter(new NoCacheFilter());  
	...
	public HttpModifiedResponse( final HttpResponse response,
	                             final boolean isSSL,
	                             String fileExtension,
	                             int responseCode ) {
	...
	private boolean isJs() {
		...
		if ( "js".equalsIgnoreCase( fileExtension ) || "js.dsp".equalsIgnoreCase( fileExtension ) ) {
			return true;
		}
		return false;
	}
	...
	private boolean isHTML() {
		...
		if ( contentType == null || contentType.toLowerCase().indexOf( "text/plain" ) != -1 || contentType.toLowerCase()
		                                                                                                  .indexOf(
				                                                                                                  "text/xml" ) != -1 ) {
	...
	private boolean hasHtmlContent() {
		String s = getSampleContent();
		return s.indexOf( "<html" ) != -1 || s.indexOf( "<body" ) != -1 || s.indexOf( "<table" ) != -1 || s.indexOf(
				"<script" ) != -1 || s.indexOf( "<form" ) != -1;
	}
	...
	public String charset() {
		if ( charset == null ) {
			...
			if ( lookIn == null ) {
				lookIn = getSampleContent( "iso-8859-1" );
			}
...
//	        	System.out.println("in.markSupported()="+in.markSupported());
				...
				System.out.println(
						"change response.sample_length in sahi.properties to a nearby value (450), restart Sahi and check." );
				try {
					in.reset();
				}
				catch ( Exception e2 ) {
