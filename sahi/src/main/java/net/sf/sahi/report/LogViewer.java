...
//        TimeDiff.diffAndRemove("page creation");
	...
	public static String highlight( final String data, final int lineNumber ) {
		StringBuffer sb = new StringBuffer();
		sb.append(
				"<html><head><meta http-equiv=\"content-type\" content=\"text/html; charset=UTF-8\" /></head><body><style>b{background:brown;color:white;}\nspan{background:lightgrey;}</style><pre>" )
		  .append( highlightLine( data, lineNumber ) ).append( "</pre></body></html>" );
		return sb.toString();
	}
	...
	public static String addLineNumbers( String orig, String scriptPath ) {
		...
		for ( int i = 0; i < len; i++ ) {
			...
			sb2.append( "<a style=\"text-decoration:none;color:inherit;\" href='/_s_/dyn/Log_highlight?href=" + Utils
					.escapeDoubleQuotesAndBackSlashes(
							scriptPath ) + "&n=" + ( i + 1 ) + "#selected'>" + line + "</a>\n" );
		}
		return sb2.toString();
	}
