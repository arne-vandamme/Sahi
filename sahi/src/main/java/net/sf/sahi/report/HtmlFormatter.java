...
public class HtmlFormatter implements Formatter
{
	...
	public String getStringResult( final TestResult result ) {
		...
		if ( !Utils.isBlankOrNull( title ) ) {
			sb.append( "\" href=\"/_s_/dyn/" ).append( Command.LOG_HIGHLIGHT ).append( "?href=" ).append( title )
			  .append( "#selected" );
		}
		sb.append( "\" title=\"" + title + "\">" ).append( result.message.trim().replaceAll( "\n", "<br/>" ) )
		  ...
		  .append( "</a>" +
				           " <span class='extra'> at " + DateFormat.getDateTimeInstance().format(
				  result.time ) + "</span>" +
				           "</div>" );
	...
	public String getSummaryData( TestSummary summary ) {
		StringBuffer sb = new StringBuffer();
		int successRate = summary.getSteps() != 0 ? ( ( summary.getSteps() - ( summary.getFailures() + summary
				.getErrors() ) ) * 100 ) / summary.getSteps()
				: 100;
		sb.append( "<tr class=\"" );
		sb.append( summary.hasFailed() ? ResultType.FAILURE.getName() : ResultType.SUCCESS.getName() );
		sb.append( "\"><td>" );
		if ( summary.addLink() ) {
			sb.append( "<a class=\"SCRIPT\" href=\"" ).append( getFileName( summary.getLogFileName() ) ).append( "\">" )
			  .append( summary.getScriptName() ).append(
					"</a>" );
		}
		else {
