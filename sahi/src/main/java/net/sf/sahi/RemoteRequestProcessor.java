...
public class RemoteRequestProcessor
{
	...
	static {
		try {
			...
			sslContext.init( SSLHelper.getKeyManagerFactoryForRemoteFetch().getKeyManagers(),
			                 SSLHelper.getAllTrustingManager(), new java.security.SecureRandom() );
		...
		try {
			...
			try {
				...
				if ( logger.isLoggable( Level.FINER ) ) {
					logger.finer( "isGZIP=" + isGZIP + "; connection.getContentEncoding()=" + connection
							.getContentEncoding() );
				}
				if ( isGZIP ) {
					if ( logger.isLoggable( Level.FINE ) ) {
				...
				if ( requestFromBrowser.isAjax() && responseCode > 300 && responseCode < 308 ) {
					...
					if ( redirectedTo != null ) {
						session.addAjaxRedirect( redirectedTo );
					}
				}
			...
			if ( responseCode != 204 && responseCode != 304 ) {
				...
				if ( responseCode == 200
					...
					if ( logger.isLoggable( Level.INFO ) ) {
						...
						sb.append( "\nContent-Disposition=" + response.getLastSetValueOfHeader(
								"Content-Disposition" ) );
...
//			}
	...
	private HttpResponse getResponse( InputStream inputStreamFromHost,
	...
	private HttpResponse getWrappedResponse( HttpResponse response ) {
		if ( useStreaming ) {
			return new StreamingHttpResponse( response );
		}
		return response;
	}

	private HttpResponse addFilters( HttpRequest requestFromBrowser,
	                                 boolean modify,
...
//			streamingResponse.addFilter(new TrafficLoggerFilter(TrafficLogger.getLoggerForThread()));
			...
			if ( modify && !requestFromBrowser.isExcluded() && !session.isAjaxRedirect( requestFromBrowser.url() ) ) {
				return new HttpModifiedResponse2( response, requestFromBrowser.isSSL(),
				                                  requestFromBrowser.fileExtension(), responseCode );
			}
		}
		return response;
	}
	...
	private HttpResponse process401( HttpURLConnection connection, InputStream inputStreamFromHost )
		...
		if ( wwwAuthenticate != null ) {
			...
			response = getWrappedResponse( new HttpFileResponse( Configuration.getHtdocsRoot() + "/spr/401.htm", props,
			                                                     false, false ) );
	...
	String getRealm( String wwwAuthenticate ) {
		...
		if ( ixComma == -1 ) {
			ixComma = wwwAuthenticate.length();
		}
		wwwAuthenticate = wwwAuthenticate.substring( ix, ixComma ).trim();
		if ( wwwAuthenticate.startsWith( "\"" ) ) {
			wwwAuthenticate = wwwAuthenticate.substring( 1 );
		}
		if ( wwwAuthenticate.endsWith( "\"" ) ) {
			wwwAuthenticate = wwwAuthenticate.substring( 0, wwwAuthenticate
					.length() - 1 );
		}
	...
	String getScheme( String wwwAuthenticate ) {
		...
		if ( ix == -1 ) {
			return wwwAuthenticate.toLowerCase();
		}
