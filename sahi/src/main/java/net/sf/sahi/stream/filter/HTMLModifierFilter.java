...
public class HTMLModifierFilter extends StreamFilter
{
	...
	private static void initializeInjects() {
		INJECT_TOP = new String( Utils.readFileAsString( Configuration.getInjectTop() ) ).replace( "$commonDomain",
		                                                                                           Configuration
				                                                                                           .getCommonDomain() );
...
//		initializeInjects();
	...
	private byte[] injectSahiFiles( byte[] data, String toInject ) throws UnsupportedEncodingException {
		...
		if ( ix == -1 ) {
			...
			injected = true;
			leftOver = "";
			return getBytesInCharset( inject( sample, ix + 1,
			                                  toInject ) ); // Have to check for ix==-1 to make sure we wait for the html/head tags
		}
	}
	...
	private int getTagEndIndex( String s ) {
		...
		if ( scriptEndIx == -1 ) {
			scriptEndIx = s.length();
		}
		int htmlIx = getTagEndIndex( s, "<html" );
		if ( scriptStartIx != -1 && scriptStartIx < htmlIx && htmlIx < scriptEndIx ) {
			htmlIx = -1;
		}
		int headIx = getTagEndIndex( s, "<head" );
		if ( scriptStartIx != -1 && scriptStartIx < headIx && headIx < scriptEndIx ) {
			headIx = -1;
		}
