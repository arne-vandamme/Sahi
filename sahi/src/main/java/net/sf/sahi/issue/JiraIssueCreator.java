...
public class JiraIssueCreator implements IssueCreator
{
	...
	void initializeXmlRpcClient( XmlRpcClient rpcClient, XmlRpcClientConfigImpl configImpl ) {
		try {
			URL jiraRpcUrl = new URL( properties.getProperty( "jira.url" ) + properties.getProperty(
					"jira.rpc.path" ) );
	...
	Map getIssueParameters() {
		...
		try {
			String projectKey = new JiraItem( "jira1.getProjects", null, properties.getProperty( "jira.project" ) ).get(
					"key" );
			issueParams.put( "project", projectKey );
			issueParams.put( "type", new JiraItem( "jira1.getIssueTypes", null, properties.getProperty(
					"jira.issueType" ) ).get( "id" ) );
			issueParams.put( "priority", new JiraItem( "jira1.getPriorities", null, properties.getProperty(
					"jira.priority" ) ).get( "id" ) );
			issueParams.put( "assignee", properties.getProperty( "jira.assignee" ) );
			List param = new ArrayList();
			param.add( projectKey );
			issueParams.put( "components", new JiraItem( "jira1.getComponents", param, properties.getProperty(
					"jira.component" ) ).getMapInList( "id" ) );
			if ( !Utils.isBlankOrNull( properties.getProperty( "jira.affectsVersions" ) ) ) {
				issueParams.put( "affectsVersions", new JiraItem( "jira1.getVersions", param, properties.getProperty(
						"jira.affectsVersions" ) ).getMapInList( "id" ) );
			}
			if ( !Utils.isBlankOrNull( properties.getProperty( "jira.fixVersions" ) ) ) {
				issueParams.put( "fixVersions", new JiraItem( "jira1.getVersions", param, properties.getProperty(
						"jira.fixVersions" ) ).getMapInList( "id" ) );
			}
		}
