...
public class TestRunner
{
	...
	public static boolean mainCLIParams( String[] args ) throws IOException, InterruptedException {
		...
		if ( browserType2 != null ) {
			...
			testRunner = new TestRunner( testName, map.get( "browser" ), map.get( "baseURL" ),
			                             host, port2, map.get( "threads" ), map.get( "browserOption" ), map.get(
					"browserProcessName" ) );
		}
	...
	protected static void help() {
		System.out.println( "------------------------" );
		System.out.println(
				"New Usage 1: java -cp /path/to/ant-sahi.jar net.sf.sahi.test.TestRunner -test <test_or_suite_name> -browserType <browser_type> -baseURL <start_url> -threads <number_of_threads>" );
		...
		System.out.println(
				" -useSingSession \t\ttrue or false. Execute all scripts sequentially in a single browser session. Default is false." );
		System.out.println( " -extraInfo \t\tAny extra info that may be accessed using _extraInfo()" );

		System.out.println( "--- OR ---" );
		System.out.println(
				"Usage: java -cp /path/to/ant-sahi.jar net.sf.sahi.test.TestRunner <test_or_suite_name> <browser_executable> <start_url> <log_dir> <sahi_host> <sahi_port> <number_of_threads> <browser_executable> [<browser_option>]" );
	...
	protected String buildURL( String command ) throws UnsupportedEncodingException {
		StringBuffer urlStr = new StringBuffer( 200 ).append( "http://" ).append( sahiHost ).append( ":" ).append(
				port ).append(
		...
		for ( Iterator<Report> iterator = listReport.iterator(); iterator.hasNext(); ) {
			...
			urlStr.append( "&" ).append( encode( report.getType() ) ).append( "=" ).append(
					report.getLogDir() != null ? encode( report.getLogDir() ) : "" );
		}
		if ( createIssue != null ) {
